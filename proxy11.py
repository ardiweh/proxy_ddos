...
def predict_packet(packet_metadata):
    try:
        ...
        print(f"[INFO] Prediction: {prediction[0]}")  # Print hasil prediksi klasifikasi
        return prediction
    except Exception as e:
...
# Fungsi untuk memproses log temporary setiap 10 detik
        ...
        try:
            for metadata in captured_packets:
                ...
                if prediction is not None and prediction[0] == 1:
                    ...
                    print(f"[INFO] No DDoS detected for packet: {metadata}")
        except Exception as e:
            print(f"[ERROR] Error in processing captured packets: {e}")
        finally:
            captured_packets = []  # Kosongkan log temporary

    # Jadwalkan ulang fungsi ini untuk berjalan lagi dalam 10 detik
    threading.Timer(10, process_captured_packets).start()

# Mulai fungsi proses log setiap 10 detik
